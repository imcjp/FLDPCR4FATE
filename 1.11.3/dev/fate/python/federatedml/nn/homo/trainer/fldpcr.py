...
class FLDPCR(TrainerBase):
    ...
    def getLocalPartyId(self):
        if self.component_properties is None:
            return -1
    ...
    def dp_train_an_epoch(self, epoch_idx, model, train_set, optimizer, loss_func):
        ...
        if isinstance(self._pri_train_loader.sampler, DistributedSampler):
            self._pri_train_loader.sampler.set_epoch(epoch_idx)

        dl = self._pri_train_loader
        ...
        for _batch_iter in to_iterate:
            ...
            if not loss_func and hasattr(pred, "loss"):
                ...
                raise ValueError(
                    'FLDPCR requires a loss function, but got None, please specify loss function in the'
                    ' job configuration')
    ...
    def train(
        ...
        for i in range(self.epochs):
            ...
            modelDP = self._pri_model
            optimizerDP=self._pri_optimizer
            epoch_loss = self.dp_train_an_epoch(i, modelDP, train_set, optimizerDP, self._loss_fn)
            # TODO cjp:
            epsUsed, best_alpha = self.privacy_engine.accountant.get_privacy_spent(
                delta=self.delta
            )
            ...
            if self.save_freq is not None and ((i + 1) % self.save_freq == 0):
                if self._deepspeed_zero_3:
                    deepspeed_util.gather_model(self._pri_model)
